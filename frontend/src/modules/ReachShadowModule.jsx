...
// Reach-Predictor Paper-Shadow — eigenständig, Richtung+TP+SL aus Coin-Historie, kein Echtgeld.
export default function ReachShadowModule() {
  ...
  useEffect(() => {
    let alive = true
    const load = () => api.get('/api/v1/reach/paper')
  ...
  return (
    <div style={{ padding: 12, color: '#ddd', fontSize: 13, overflow: 'auto', height: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
        <strong>Reach-Predictor · Paper-Shadow</strong>
        <span style={{ fontSize: 11, color: '#888' }}>
          Gate: Netto ≥ {gate.min_net_expectancy_pct}% · Reach ≥ {gate.min_reach_rate} · ${d.trade_size_usd}/Trade · kein Echtgeld
        </span>
      </div>
      ...
      <div style={{ background: '#15151b', borderRadius: 6, padding: '8px 10px', marginBottom: 12, fontSize: 12 }}>
        <span style={{ color: '#888' }}>Richtung + TP/SL kommen aus der Coin-Historie (alle Features, kein Random). </span>
        Es wird nur getradet, wenn die <b style={{ color: '#9c9' }}>Netto-Expectancy</b> nach Gebühr positiv ist
        und genug ähnliche Fälle das TP <b style={{ color: '#9c9' }}>vor</b> dem SL erreichten.
        <span style={{ color: '#666' }}> — Edge ist dünn: die Equity-Kurve entscheidet, nicht die Hoffnung.</span>
      </div>
          ...
          <tbody>
            {(d.open_positions || []).map((p, i) => (
              <tr key={i} style={{ borderTop: '1px solid #2a2a33' }}>
                ...
                <td style={{ color: '#6a9' }}>+{p.tp_pct}</td>
                <td style={{ color: '#a66' }}>−{p.sl_pct}</td>
                <td>{Number(p.conf).toFixed(2)}</td>
                <td style={{ color: '#888' }}>{p.n_match}</td>
