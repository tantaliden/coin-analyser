...
def pull_rows_for_slots(cfg: dict, slots: list) -> list:
    ...
    for line in result.stdout.strip().split("\n"):
        ...
        rows.append({
            ...
            "bbo_bid_px": float(parts[19]) if parts[19] else None,
            "bbo_ask_px": float(parts[20]) if parts[20] else None,
            "bbo_bid_sz": float(parts[21]) if parts[21] else None,
            "bbo_ask_sz": float(parts[22]) if parts[22] else None,
            "spread_bps": float(parts[23]) if parts[23] else None,
            "book_imbalance_5": float(parts[24]) if parts[24] else None,
            "book_depth_5": float(parts[25]) if parts[25] else None,
            "book_imbalance_10": float(parts[26]) if parts[26] else None,
            "book_depth_10": float(parts[27]) if parts[27] else None,
            "book_imbalance_20": float(parts[28]) if parts[28] else None,
            "book_depth_20": float(parts[29]) if parts[29] else None,
        })
    return rows
