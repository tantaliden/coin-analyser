"""Sequenz-Modell Paper-Shadow API (Phase 3, read-only).
Eigenes Paper-Buch (seq_paper_positions) mit Live-Werten (akt. Preis + unrealisierter
PnL der offenen Positionen, analog Paper-Wallet) + Historie + Equity-Kurve.
...
router = APIRouter(prefix="/api/v1/seq", tags=["seq"])
...
def _current_prices(symbols):
    ...
    dieselbe Quelle wie die Paper-Wallet. KEIN Fallback: fehlt der Preis -> None."""
...
def seq_paper():
    ...
    with _db_app() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute("""SELECT closed_at, pnl_usd FROM seq_paper_positions
    ...
    # Live-Werte der offenen Positionen
    ...
    return {
        "conf_threshold": lv["conf_threshold"], "trade_size_usd": size,
        "tp_pct": c["labeling"]["tp_pct"], "sl_pct": c["labeling"]["sl_pct"],
        ...
        "history": history,
        "equity_curve": curve,
        "backtest_expectation": {"avg_pnl_pct_per_trade": 0.206, "tp_rate_pct": 39.1,
                                 "signals_per_day": 145, "note": "v1 @ conf>=0.95, Gate-2 OOS"},
    }
