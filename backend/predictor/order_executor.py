...
def execute_order_with_failsafe(creds, symbol, is_long, leverage, size_usd,
    ...
    from rl_agent.trader import (
        place_limit_order_hl, place_tp_sl_hl,
    ...
    # === Schritt 1: Position oeffnen (size_usd = NOTIONAL!) ===
    # Slippage min. 1% — bei volatilen Markten verhindert das, dass die
    # Open-Order bei spike unfilled bleibt.
    open_slippage = max(float(slippage_pct), 1.0)
    order_res = place_limit_order_hl(
        creds, symbol, is_buy=is_long, size_usd=notional,
        price=entry_px, leverage=effective_lev, slippage_pct=open_slippage,
    )
    ...
    # Resting-Fix: place_limit_order_hl returnt success=True auch wenn die Order
    # nur im Orderbuch liegt und nie gefuellt wird. Wir wollen nur gefuellte Orders.
    # Resting-Order canceln (sonst spukt sie weiter im Buch) und als Fail zurueck.
    if order_res.get("status") != "filled":
        oid = order_res.get("order_id")
        if oid:
            try:
                from rl_agent.trader import cancel_order_hl
                cancel_order_hl(creds, symbol, int(oid))
            except Exception as e:
                log.warning("cancel resting order %s/%s scheiterte: %s", symbol, oid, e)
        return {"success": False, "error": f"order_not_filled (status={order_res.get('status')})",
                "order": order_res, "effective_leverage": effective_lev}

    # WICHTIG: TP/SL gegen ACTUAL fill-price (avg_price) neu rechnen, nicht
    # gegen entry_px (mid-Lookup). Sonst landen TP/SL bei volatilem Markt am
    # falschen Niveau (z.B. TP unter dem Open-Fill -> sofort als Loss gefuellt).
