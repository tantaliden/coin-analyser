...
async def get_hl_balance(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Spot-Balance zusaetzlich (USDC auf Spot-Account)
        spot_usdc = 0.0
        try:
            ...
            print(f"[WALLET-HL] spot-state failed: {se}")
        total_combined = account_value + spot_usdc
        return {
            ...
            "total_combined": round(total_combined, 2)
        }
    except Exception as e:
