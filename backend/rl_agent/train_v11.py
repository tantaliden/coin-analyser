#!/usr/bin/env python3
"""
RL-Agent V11 Training — 512er Netz, kein SL, kein Timeout, 15 Slots.

Phase 1: Jan - Maerz 2025 — Markt erforschen (5m, einfaches +/- PnL)
Phase 2: Apr 2025 - Dez 2025 — Traden lernen (5m, volles Reward-System)
Phase 3: Jan 2026 - 20. Maerz 2026 — Feinschliff (5m, niedrigere LR)

512-512 MLP, env_v11 (kein SL, kein Timeout, 15 Positionen, 2000 Startpunkte).
Speichert als rl_ppo_trading_v11.zip.

Usage:
    cd /opt/coin/backend
    nohup /opt/coin/venv/bin/python3 -u rl_agent/train_v11.py > /opt/coin/logs/rl_train_v11.log 2>&1 &
"""
import sys
import json
import time
...
from rl_agent.env_v11 import TradingEnvV11

SETTINGS_PATH = "/opt/coin/settings.json"
MODEL_DIR = Path("/opt/coin/database/data/models")
MODEL_PATH = MODEL_DIR / "rl_ppo_trading_v11.zip"
...
class PhaseCallback(BaseCallback):
    ...
    def _log(self):
        ...
        wins = getattr(self.env, 'total_wins', 0)
        losses = getattr(self.env, 'total_losses', 0)
        winrate = wins / (wins + losses) * 100 if (wins + losses) > 0 else 0
        hold_min = getattr(self.env, 'total_hold_minutes', 0)
        avg_hold = hold_min / total_trades if total_trades > 0 else 0
        print(f"\n[{self.phase_name}] {self.num_timesteps:,} steps | "
              f"Punkte: {self.env.total_points:.0f} | "
              f"Trades: {total_trades} | "
              f"WinRate: {winrate:.1f}% | "
              f"Avg Hold: {avg_hold:.1f}m | "
        ...
        for lev in range(1, 11):
            ...
            if s['trades'] > 0:
                ...
                print(f"  {lev:2d}x: {s['trades']:6d} Trades, Avg {avg:+.3f}, WR {wr:.1f}%")
        print(flush=True)
...
def preload_market_data(coins_conn, symbols, data_start, trade_end):
    """Market-Daten laden (kein 1m noetig bei 5m Management)."""
...
def run():
    ...
    print("=" * 70)
    print("  RL-Agent V11 Training — 512er, kein SL, kein Timeout, 15 Slots")
    print("  Phase 1: Markt erforschen (Jan-Maerz 2025, 5m)")
    print("  Phase 2: Traden lernen (Apr 2025-Dez 2025, 5m)")
    print("  Phase 3: Feinschliff (Jan-20.Maerz 2026, 5m)")
    print("  512-512 MLP, env_v11 (kein SL, kein Timeout, 15 Pos, 2000 Start)")
    ...
    # Neues Modell from scratch — 512-512 MLP
    dummy_env = TradingEnvV11(
        coins, {'agg_5m': {}, 'agg_1h': {}, 'agg_4h': {}, 'agg_1d': {}, 'kline_metrics': {}},
    ...
    print("  Neues PPO-Modell erstellt: 512-512 MLP\n")
    ...
    for i, phase in enumerate(PHASES):
        ...
        market_data = preload_market_data(coins_conn, coins, phase['data_start'], phase['trade_end'])
        coins_conn.close()
        ...
        # Phase speichern
        phase_path = MODEL_DIR / f"rl_ppo_trading_v11_phase{i+1}.zip"
    ...
    print(f"\n[V11] Finales Modell gespeichert: {MODEL_PATH}")
