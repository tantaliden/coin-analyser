...
def _calc_trade_size(wallet_balance, base_size=BASE_TRADE_SIZE):
    """Progressive Trade-Size (Notional)."""
...
def _finalize_close(app_conn, state, pos, current_price, exit_price, pnl_pct, exit_reason):
    ...
    # Experience Buffer: Trade-Zeitraum merken fuer Lernzyklus
    et = entry_time
...
def manage_positions(model, app_conn, coins_conn, state, creds):
    ...
    for pos in open_positions:
        ...
        # W-V1.0 hat 59 Features — 2 Extra-Features anhängen
        import numpy as _np
        obs_ext = _np.zeros(WALLET_N_FEATURES, dtype=_np.float32)
        obs_ext[:N_FEATURES] = obs[:N_FEATURES]
        obs_ext[57] = state.get('weekly_target_points', 0) / 50.0
        week_start = state.get('week_start_points', state.get('total_points', 2000))
        obs_ext[58] = (state.get('total_points', 2000) - week_start) / max(week_start, 1)

        mgmt_action, _ = model.predict(obs_ext, deterministic=False)
        mgmt_action = int(mgmt_action)

        # Verkaufs-Block: -0.5% bis +0.5%
        if mgmt_action != 0 and -0.5 <= pnl_pct <= 0.5:
            mgmt_action = 0
...
def scan_all_coins(model, app_conn, coins_conn, state, creds, tradeable_coins):
    ...
    max_concurrent = MAX_CONCURRENT_FULL if wallet_balance >= MAX_CONCURRENT_THRESHOLD else MAX_CONCURRENT_START
    trades_opened = 0
    ...
    for symbol in shuffled_coins:
        ...
        try:
            ...
            print(f"[RL-AGENT] ALARM: HL API nicht erreichbar! Kein Trading. ({e})")
            _send_telegram_alarm("HL API nicht erreichbar! Kein Coin-Scan.")
            return 0
...
def experience_learn(model, experiences):
    ...
    Agent trainiert auf den Coins und Zeitraeumen wo er tatsaechlich gehandelt hat.
    Kein Unterschied zwischen Training und Live.
    """
    try:
        from rl_agent.learner import preload_market_data

        # Zeitraum und Coins aus echten Trades
        ...
        # BTC + ETH immer dabei (Referenz-Features)
        all_coins = traded_coins[:]
        for ref in ['BTCUSDC', 'ETHUSDC']:
            if ref not in all_coins:
                all_coins.append(ref)

        data_start = earliest - timedelta(hours=12)  # Vorlauf fuer Features
        ...
        print(f"[RL-AGENT] EXPERIENCE-LERNEN: {LEARN_STEPS} Steps | "
              f"{len(experiences)} Trades | {len(traded_coins)} Coins | "
              f"{span_h:.1f}h Zeitraum | Env-Punkte: {env.total_points:.0f}")
...
def main():
    ...
    print(f"  {sl_info} | {to_info} | Max {MAX_CONCURRENT_START} Positionen")
    ...
    while running:
        try:
            ...
            # 4. Alle 5 Min: Coin-Scan fuer neue Trades
