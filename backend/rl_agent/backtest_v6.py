#!/usr/bin/env python3
"""
RL-Agent Backtest V6 — Autonomous Agent, 1-Min Management.

Wie V5 aber: Positions-Management im 1-Minuten-Takt.
Entry-Scan alle 5 Min, Management jede 1 Min.
klines_1m fuer praezise Preise in Position.

Zeitraum: 01.01.2026 -> 14.03.2026

Usage:
    cd /opt/coin/backend
    nohup python3 -u rl_agent/backtest_v6.py > /opt/coin/logs/backtest_v6.log 2>&1 &
"""
import sys
import json
import time
...
RESULTS_PATH = "/opt/coin/logs/rl_backtest_v6_results.json"
TRADES_LOG_PATH = "/opt/coin/logs/rl_backtest_v6_trades.json"
MODEL_PATH = "/opt/coin/database/data/models/rl_ppo_trading_v6.zip"
...
MAX_CONCURRENT_THRESHOLD = 1500.0  # Nicht mehr relevant, beide gleich
...
def _get_close_1m(market_data, symbol, current_time):
    """Preis aus klines_1m, Fallback agg_5m."""
    data = market_data.get('klines_1m', {}).get(symbol)
    if data:
        idx = bisect.bisect_right(data['timestamps'], current_time) - 1
        if 0 <= idx < len(data['close']):
...
def _get_close_5m(market_data, symbol, current_time):
    data = market_data.get('agg_5m', {}).get(symbol)
    if not data:
        return 0.0
    idx = bisect.bisect_right(data['timestamps'], current_time) - 1
...
def _check_sl(market_data, symbol, current_time, entry_price, direction):
    """SL-Check mit klines_1m, Fallback agg_5m."""
    data = market_data.get('klines_1m', {}).get(symbol)
    if not data:
        data = market_data.get('agg_5m', {}).get(symbol)
    if not data:
...
def run_backtest():
    ...
    print("=" * 70)
    print("  RL-Agent Backtest V6 — Autonomous, 1-Min Management")
    print(f"  Zeitraum: {TEST_START.date()} -> {TEST_END.date()}")
    print(f"  Budget: ${START_BALANCE}, Trade: ${BASE_TRADE_SIZE}, progressiv")
    print(f"  Max Positionen: {MAX_CONCURRENT_START} -> {MAX_CONCURRENT_FULL} (ab ${MAX_CONCURRENT_THRESHOLD:.0f}), Hebel 1x-10x")
    print(f"  Max Haltezeit: {MAX_HOLD_MINUTES}min, SL: -{SL_PERCENT}%")
    print(f"  Entry: alle {ENTRY_STEP_MINUTES} Min, Management: jede {MGMT_STEP_MINUTES} Min")
    print("=" * 70)
    print()
    ...
    # V6 Modell laden
    env = TradingEnvV6(coins, market_data, TEST_START, TEST_END, train_start=TEST_START)
    try:
        model = PPO.load(MODEL_PATH, env=env)
        print(f"  V6 Modell geladen: {MODEL_PATH}")
    except Exception as e:
        print(f"FEHLER: V6 Modell laden fehlgeschlagen: {e}")
        return

    print()
    print("=" * 70)
    print("  Backtest (1-Min Management, 5-Min Entry-Scan)")
    print("=" * 70)
    print()
    ...
    while current_time < TEST_END:
        ...
        # Drawdown-Modus deaktiviert — Agent entscheidet selbst
        # drawdown_mode = state['week_points'] < 0

        # === 1. Offene Positionen managen (jede 1 Min) ===
        still_open = []
        closed_coins = set()
        for pos in open_positions:
            ...
            price = _get_close_1m(market_data, pos['symbol'], current_time)
            if price <= 0:
                still_open.append(pos)
                continue
        ...
        # === 2. Neue Trades — alle 5 Min ===
        if (current_time - last_scan_time).total_seconds() >= ENTRY_STEP_MINUTES * 60:
            # Tages-Trade-Limit
            today_str = current_time.strftime('%Y-%m-%d')
            if today_str != current_trade_day:
                current_trade_day = today_str
                trades_today = 0

            n_open = len(open_positions)
            open_symbols = {p['symbol'] for p in open_positions}
            current_portfolio = portfolio - total_margin_locked
            max_concurrent = MAX_CONCURRENT_FULL if current_portfolio >= MAX_CONCURRENT_THRESHOLD else MAX_CONCURRENT_START
            ...
            for symbol in coins:
                ...
                price = _get_close_5m(market_data, symbol, current_time)
                if price <= 0:
                    continue
        ...
        current_time += timedelta(minutes=MGMT_STEP_MINUTES)

    # Restliche Positionen schliessen
    for pos in open_positions:
        price = _get_close_1m(market_data, pos['symbol'], TEST_END)
        ...
        state['week_points'] += reward
        state['week_points_raw'] += reward
        duration_min = int((TEST_END - pos['entry_time']).total_seconds() / 60)
        pnl_dollar = pos['margin'] * pos['leverage'] * pnl_pct / 100
        fees = pos['margin'] * pos['leverage'] * FEE_RATE * 2
        portfolio += pos['margin'] + pnl_dollar - fees
        total_margin_locked -= pos['margin']
        trade_log.append({
            ...
            'margin': round(pos['margin'], 2), 'leverage': pos['leverage'],
            'duration_min': duration_min, 'exit_reason': 'end_of_test',
            'reward': round(reward, 2), 'total_points': round(state['total_points'], 1),
        })
    ...
    print(f"--- Uebersicht ---")
    print(f"  Coin-Praesentationen: {total_skips + total_entries}")
    ...
    if trade_log:
        ...
        print(f"  Avg Hebel:           {avg_lev:.1f}x")
        print()
        ...
        print(f"  Avg Reward/Trade:    {np.mean(rewards):+.2f}")
        print(f"  Positive Rewards:    {len(pos_rewards)} (Avg {np.mean(pos_rewards):+.2f})" if pos_rewards else "  Positive Rewards:    0")
        print(f"  Negative Rewards:    {len(neg_rewards)} (Avg {np.mean(neg_rewards):+.2f})" if neg_rewards else "  Negative Rewards:    0")
        print()
        ...
        for reason, count in sorted(exit_reasons.items(), key=lambda x: -x[1]):
            ...
            print(f"  {reason:<16} {count:>6} ({pct:.1f}%)  Avg PnL: {avg:+.3f}%  ${dollar:+.2f}")
        print()
