#!/usr/bin/env python3
"""
RL-Agent Backtest V3 — PPO Discrete(21) auf CNN-Predictions.

Lädt V3-Modell (Discrete(21), 1x-10x Hebel, 3-Stufen-Reward).
...
Usage:
    cd /opt/coin/backend
    nohup python3 -u rl_agent/backtest_v3.py > /opt/coin/logs/rl_backtest_v3.log 2>&1 &
"""
import sys
import json
import time
...
RESULTS_PATH = "/opt/coin/logs/rl_backtest_v3_results.json"
TRADES_LOG_PATH = "/opt/coin/logs/rl_backtest_v3_trades.json"
...
def _calc_trade_size(portfolio):
    """Progressive Trade-Size: $15 flat → 1/100 ab $1500 → 1/110 ab $5000 → ... → 1/200 ab $14000+."""
    if portfolio < 1500:
        return BASE_TRADE_SIZE
    if portfolio < 5000:
        return portfolio / 100
    # Ab $5000: Divisor steigt um 10 pro $1000 Stufe
    step = int((portfolio - 5000) / 1000)  # 0 bei $5000, 1 bei $6000, ...
    divisor = 110 + step * 10
    if divisor > 200:
        divisor = 200
...
def run_backtest():
    ...
    print("=" * 70)
    print("  RL-Agent Backtest V3 — Discrete(21), 3-Stufen-Reward")
    print(f"  Zeitraum: {TEST_START.date()} → {TEST_END.date()}")
    print(f"  Budget: ${START_BALANCE}, Trade: ${BASE_TRADE_SIZE}, ab $1500 → 1/100, ab $5000 → progressiv bis 1/200")
    ...
    # === V3 Modell laden + Weiterlernen ===
    MODEL_V3_PATH = "/opt/coin/database/data/models/rl_ppo_trading_v3.zip"
    env = TradingEnvV3(opportunities, market_data, train_start=TEST_START)

    try:
        model = PPO.load(MODEL_V3_PATH, env=env)
        model.learning_rate = 0.0003
        model.lr_schedule = lambda _: 0.0003
        print(f"\n  V3 Modell geladen: {MODEL_V3_PATH}")
        print(f"  LR: 0.0003")
    except Exception as e:
        print(f"FEHLER: V3 Modell laden fehlgeschlagen: {e}")
        return
    ...
    while current_time < TEST_END:
        ...
        for pos in open_positions:
            ...
            if price <= 0:
                pnl_pct = 0.0
                reward = 0.0
                bonus = _point_bonus(total_points)
                reward_with_bonus = reward * bonus
                total_points += reward_with_bonus
                ...
                trade_log.append({
                    ...
                    'reward': round(reward, 2), 'bonus': bonus, 'total_points': round(total_points, 1),
                })
                continue
        ...
        if slot in pred_by_time:
            for pred_idx in pred_by_time[slot]:
                ...
                # V3: Action 1-10 = long, 11-20 = short, Hebel aus LEVERAGE_MAP
    ...
    if trade_log:
        ...
        print(f"  Endpunkte:           {total_points:+.0f}")
