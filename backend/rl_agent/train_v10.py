#!/usr/bin/env python3
"""
RL-Agent V10 Training — 512er Netz, 3 Phasen, from scratch.
...
512-512 MLP, env_v8 (Kapazitaets-Strafe), from scratch.
Speichert als rl_ppo_trading_v10.zip.

Usage:
    cd /opt/coin/backend
    nohup python3 -u rl_agent/train_v10.py > /opt/coin/logs/rl_train_v10.log 2>&1 &
"""
import sys
import json
import time
...
from rl_agent.env_v8 import TradingEnvV6 as TradingEnvV10

SETTINGS_PATH = "/opt/coin/settings.json"
MODEL_DIR = Path("/opt/coin/database/data/models")
MODEL_PATH = MODEL_DIR / "rl_ppo_trading_v10.zip"
...
def run():
    ...
    print("=" * 70)
    print("  RL-Agent V10 Training — 512er Netz, from scratch")
    ...
    print("  512-512 MLP, env_v8 (Kapazitaets-Strafe)")
    ...
    for i, phase in enumerate(PHASES):
        ...
        # Phase speichern
        phase_path = MODEL_DIR / f"rl_ppo_trading_v10_phase{i+1}.zip"
    ...
    print(f"\n[V10] Finales Modell gespeichert: {MODEL_PATH}")
