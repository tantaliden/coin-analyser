#!/usr/bin/env python3
"""
RL-Agent Backtest V4 — PPO Discrete(21) mit Serien-Mechanik.

Identisch zu V3-Backtest, aber:
  - V4-Modell (rl_ppo_trading_v4.zip)
  - Verlust-Serien-Strafe (ab Tag 2)
  - Wochen-Bonus (x1.20, +0.05 pro Folgewoche)
  - Drawdown-Modus (Woche im Minus → max 3x)
  - Hebel-Freischaltung DEAKTIVIERT (Backtest = nach Training, alles freigeschaltet)

Usage:
    cd /opt/coin/backend
    nohup python3 -u rl_agent/backtest_v4.py > /opt/coin/logs/rl_backtest_v4.log 2>&1 &
"""
import sys
import json
import time
...
from rl_agent.env_v4 import TradingEnvV4, LEVERAGE_MAP, _reward
from rl_agent.features import compute_observation, empty_candles, N_FEATURES

SETTINGS_PATH = "/opt/coin/settings.json"
RESULTS_PATH = "/opt/coin/logs/rl_backtest_v4_results.json"
TRADES_LOG_PATH = "/opt/coin/logs/rl_backtest_v4_trades.json"
...
def _calc_trade_size(portfolio):
    """Progressive Trade-Size: $15 flat → 1/100 ab $1500 → progressiv bis 1/200."""
    if portfolio < 1500:
...
def run_backtest():
    ...
    print("=" * 70)
    print("  RL-Agent Backtest V4 — Discrete(21), Serien-Mechanik")
    print(f"  Zeitraum: {TEST_START.date()} → {TEST_END.date()}")
    print(f"  Budget: ${START_BALANCE}, Trade: ${BASE_TRADE_SIZE}, progressiv bis 1/200")
    print(f"  Max Positionen: {MAX_CONCURRENT}, Hebel 1x-10x aus Agent-Action")
    print(f"  Agent entscheidet Richtung SELBST (kein CNN-Hint)")
    print(f"  Verlust-Serie: Ab Tag 2 → 1%/2%/4%/.../20%")
    ...
    # === Opportunities (OHNE Richtung — Agent entscheidet selbst) ===
    opportunities = []
    for p in predictions:
    ...
    # === V4 Modell laden ===
    MODEL_V4_PATH = "/opt/coin/database/data/models/rl_ppo_trading_v4.zip"
    env = TradingEnvV4(opportunities, market_data, train_start=TEST_START)

    try:
        model = PPO.load(MODEL_V4_PATH, env=env)
        print(f"\n  V4 Modell geladen: {MODEL_V4_PATH}")
    except Exception as e:
        print(f"FEHLER: V4 Modell laden fehlgeschlagen: {e}")
        return
    ...
    while current_time < TEST_END:
        ...
        for pos in open_positions:
            ...
            if price <= 0:
                ...
                trade_log.append({
                    ...
                    'reward': round(reward, 2), 'bonus': bonus, 'total_points': round(state['total_points'], 1),
                })
                continue
        ...
        if slot in pred_by_time:
            for pred_idx in pred_by_time[slot]:
                ...
                # Action → Richtung + Hebel
                agent_direction = 1 if action <= 10 else -1
                agent_dir_str = 'long' if action <= 10 else 'short'
                leverage = LEVERAGE_MAP[action]

                # Drawdown-Modus: max 3x
                if drawdown_mode and leverage > 3:
                    leverage = 3

                # Positionsgröße (progressiv)
                ...
                results.append({
                    ...
                    'margin': round(margin, 2), 'drawdown_mode': drawdown_mode,
                    'detected_at': str(current_time), 'portfolio': round(portfolio - total_margin_locked, 2),
                    'n_open': len(open_positions),
                })
    ...
    if trades_taken:
        ...
        drawdown_trades = [t for t in trades_taken if t.get('drawdown_mode')]
        ...
        print(f"  Drawdown-Modus:      {len(drawdown_trades)} Trades")
        print()
    ...
    if trade_log:
        ...
        print(f"  Ø Reward/Trade:      {np.mean(rewards):+.2f} (vor Bonus)")
        ...
        print(f"  Aktuelle Verlust-S.: {state['losing_streak_days']} Tage")
        print(f"  Aktuelle Wochen-S.:  {state['winning_streak_weeks']} Wochen")
        print()
    ...
    if trade_log:
        ...
        for t in trade_log:
            ...
            weekly[w]['points'] += t.get('reward', 0) * t.get('bonus', 1.0)
            if t['pnl_dollar'] > 0:
                weekly[w]['wins'] += 1
            elif t['pnl_dollar'] < 0:
                weekly[w]['losses'] += 1
