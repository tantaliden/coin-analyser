#!/usr/bin/env python3
"""
RL-Agent Training V2 — Erweiterter Action-Space mit Hebel.

Discrete(9): skip, long 1x/3x/5x/10x, short 1x/3x/5x/10x
Phase 1 (6 Monate): Nur Lernen (Richtung + Hebel-Risiko)
Phase 2 (12 Monate): $50/Position, max 40 gleichzeitig, kein Startkapital
...
Usage:
    cd /opt/coin/backend
    nohup python3 -u rl_agent/trainer_v2.py > /opt/coin/logs/rl_training_v2.log 2>&1 &
"""
import sys
import json
import time
...
from rl_agent.env_v2 import TradingEnvV2

SETTINGS_PATH = "/opt/coin/settings.json"
MODEL_DIR = Path("/opt/coin/database/data/models")
MODEL_PATH = MODEL_DIR / "rl_ppo_trading_v2.zip"
...
def train():
    ...
    print("=" * 70)
    print("  PPO Training V2 — Discrete(9) mit Hebel")
    ...
    env = TradingEnvV2(opps, market_data, train_start=TRAIN_START)

    # Adaptive Lernrate: KL-basiert
    # Wenn KL hoch (viel zu lernen) → LR steigt
    # Wenn KL niedrig (schon gut) → LR sinkt
    target_kl = 0.03
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=0.002,      # 6.7x höher als Standard, target_kl sichert Stabilität
        n_steps=2048,
        batch_size=64,
        n_epochs=10,
        gamma=0.99,
        ...
        clip_range=0.2,
        ent_coef=0.01,
        target_kl=target_kl,      # Stoppt Update wenn KL zu hoch → natürliche Bremse
        verbose=1,
        policy_kwargs={
            'net_arch': [256, 256],
        },
    )
    print(f"[RL-AGENT] PPO V2 Modell erstellt: 256-256 MLP, Discrete(9)")
    ...
    print(f"\n[RL-AGENT] V2 Modell gespeichert: {MODEL_PATH}")
